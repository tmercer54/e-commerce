...
export function renderOrderSummary() {
    ...
    if (!cart || cart.length === 0) {
        console.warn('Cart is empty or not defined.');
        return;
    }
    ...
    cart.forEach((cartItem) => {
        ...
        cartSummaryHTML += `
        <div class="cart-item-container js-cart-container js-cart-item-container-${matchingProduct.id}">
        ...
        document.querySelectorAll('.js-delete-link').forEach((link) => {
            link.addEventListener('click', () => {
                ...
                if (container) {
                    ...
                    console.error(`Cart item container for product ID ${productId} not found.`);
                }
